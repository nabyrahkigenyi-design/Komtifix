...
type Props = {
  className?: string;
  size?: number; // px
  /** If true: hides icons whose URL is empty (recommended). */
  hideEmpty?: boolean;
};
...
type SocialKey = "whatsapp" | "facebook" | "instagram" | "linkedin";

type BrandSocials = Partial<Record<Exclude<SocialKey, "whatsapp">, string>>;

/**
 * Put real profile URLs here (or in brand.socials if you prefer later).
 * Keep empty string to hide.
 */
const DEFAULT_SOCIALS: BrandSocials = {
  facebook: "", // e.g. "https://facebook.com/komtifix"
  instagram: "", // e.g. "https://instagram.com/komtifix"
  linkedin: "", // e.g. "https://linkedin.com/company/komtifix"
};

export default function SocialLinks({ className = "", size = 22, hideEmpty = true }: Props) {
  const { t } = useI18n();

  const links = useMemo(() => {
    const waPhone = digitsOnly(brand.phone);
    const waText = encodeURIComponent(`${brand.name}: ${t("cta_quote")}`);
    const socialsFromBrand = (brand as any).socials as BrandSocials | undefined;

    const merged: Record<SocialKey, string> = {
      whatsapp: `https://wa.me/${waPhone}?text=${waText}`,
      facebook: socialsFromBrand?.facebook ?? DEFAULT_SOCIALS.facebook ?? "",
      instagram: socialsFromBrand?.instagram ?? DEFAULT_SOCIALS.instagram ?? "",
      linkedin: socialsFromBrand?.linkedin ?? DEFAULT_SOCIALS.linkedin ?? "",
    };
  ...
  const iconClass =
    "text-white/85 hover:text-white transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--color-teal)] rounded";
  const s = { width: size, height: size };

  const shouldRender = (url: string) => (hideEmpty ? !!url : true);
